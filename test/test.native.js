...
var sminabs = tryRequire( resolve( __dirname, './../lib/native.js' ) );
var opts = {
...
tape( 'the function has an arity of 1', opts, function test( t ) {
	t.strictEqual( sminabs.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the minimum absolute value of a one-dimensional ndarray', opts, function test( t ) {
	var x;
	var v;

	x = new Float32Array( [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ] );
	v = sminabs( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 0.0, 'returns expected value' );

	x = new Float32Array( [ -4.0, -5.0 ] );
	v = sminabs( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, 4.0, 'returns expected value' );

	x = new Float32Array( [ -0.0, 0.0, -0.0 ] );
	v = sminabs( [ vector( x, 3, 1, 0 ) ] );
